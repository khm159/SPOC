...
class ReactAgent:
    ...
    def __init__(self, cfg, llm_handler):
        ...
        self.text_trajectory_save_dir = os.path.join(cfg.log.save_path,"text_trajectory")
        if not os.path.exists(self.text_trajectory_save_dir):
            os.makedirs(self.text_trajectory_save_dir)
    ...
    def run(self, env, log, instruction, init_info):
        ...
        env.instruction = instruction # set instruction 
        nl_inst_info = {
            'nl_inst': instruction, 
            'message': None
        }
        ...
        # logging initial prompt and example
        ...
        while True:
            if self.cur_step_id > self.max_steps:
                #  logging Max Step Errors 
                log.info('Max steps')
                trajectory += f'Max steps\n'
                log_data['trajectory'] = trajectory
                terminate_info = {
                    'terminate': 'max_step', 
            ...
            if action_step is None:
                ...
                # logging inferenced next step information
                trajectory += f'Think: {thought}\nOK.\n'
